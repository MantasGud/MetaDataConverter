...
public class DB2DDL implements DDLHandler {
    ...
    public void createDDLFile(MetadataToolState state, List<TableMetaData> schema, FileWriter ddlWriter) {
        ...
        switch (state.getDbType()) {
            case MYSQL, SQL, AS400 -> throw new UnsupportedOperationException("Not implemented yet.");
        }
        ...
        switch (column.getDataType().toUpperCase()) {
            case "NUMBER":
                if (column.getScale() > 0) {
                    sb.append("DECIMAL(").append(column.getLength()).append(",").append(column.getScale()).append(")");
                } else {
                    sb.append("DECIMAL(").append(column.getLength()).append(",").append("0").append(")");
                }
                break;
            case "VARCHAR2":
            case "NVARCHAR2":
